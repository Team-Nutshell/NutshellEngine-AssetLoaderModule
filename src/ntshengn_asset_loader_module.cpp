...
std::unordered_map<std::string, NtshEngn::Material> NtshEngn::AssetLoaderModule::loadMaterialMtl(const std::string& filePath) {
	...
	while (std::getline(file, line)) {
		...
		if (tokens[0] == "newmtl") {
			...
			std::string mapKey = "srgb " + tokens[1] + " " + tokens[2] + " " + tokens[3] + " " + std::to_string(1.0f);

			Image* image = assetManager->findImageByName(mapKey);
			if (!image) {
				image = assetManager->createImage(mapKey);
				image->width = 1;
				image->height = 1;
				image->format = ImageFormat::R8G8B8A8;
					...
					if (baseColorTexture) {
						...
						if (baseColorImage->uri) {
							...
							if (base64Pos != std::string::npos) {
								...
								if (result == cgltf_result_success) {
									image = assetManager->findImageByName(uriBase64);
									if (!image) {
										image = assetManager->createImage(uriBase64);
						...
						if (image) {
							if (baseColorTexture->sampler) {
								...
								primitive.material.diffuseTexture.imageSampler.maxAnisotropy = 16.0f;
							}
							else {
					...
					if (metallicRoughnessTexture) {
						...
						if (image) {
							...
							if (metallicRoughnessTexture->sampler) {
								...
								primitive.material.metalnessTexture.imageSampler.maxAnisotropy = 16.0f;
							}
							else {
				...
				if (normalTexture) {
					...
					if (image) {
						if (normalTexture->sampler) {
							...
							primitive.material.normalTexture.imageSampler.maxAnisotropy = 16.0f;
						}
						else {
				...
				if (emissiveTexture) {
					...
					if (image) {
						if (emissiveTexture->sampler) {
							...
							primitive.material.emissiveTexture.imageSampler.maxAnisotropy = 16.0f;
						}
						else {
				...
				if (occlusionTexture) {
					...
					if (image) {
						if (occlusionTexture->sampler) {
							...
							primitive.material.occlusionTexture.imageSampler.maxAnisotropy = 16.0f;
						}
						else {
